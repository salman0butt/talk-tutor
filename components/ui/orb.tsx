...
function Scene({
  ...
  const random = useMemo(() => splitmix32(seed ?? randomSeed), [randomSeed, seed])
  ...
  useFrame((_, delta: number) => {
    ...
    const time = advanceOrbMaterial(mat, delta)
  ...
  const texture = useMemo(() => {
    const clonedTexture = perlinNoiseTexture.clone()
...
function advanceOrbMaterial(material: THREE.ShaderMaterial, delta: number) {
  ...
  return uniforms.uTime.value as number
}
...
function updateOrbUniforms(
  ...
  const uniforms = material.uniforms
  uniforms.uAnimation.value += delta * animationSpeed
  uniforms.uInputVolume.value = inputVolume
  uniforms.uOutputVolume.value = outputVolume
  uniforms.uColor1.value.lerp(color1, 0.08)
  uniforms.uColor2.value.lerp(color2, 0.08)
}
